...
Para acelerar el movimiento de la nave será necesario que nuestro control **mover la nave** considere una nueva combinación de teclas, por ejemplo: **tecla control CTRL**  + **flecha arriba ↑**. 
...
.. code-block:: python
    ...
    def mover_sprites(tecla_principal, tecla_modificadora):
        ...
        if tecla_principal == arcade.key.UP and tecla_modificadora & arcade.key.LCTRL:
            nave01.center_y = nave01.center_y + (ESPACIO * 4)
    
    ...

Al ejecutar el código, presiona la combicnación de teclas **tecla control CTRL**  + **flecha arriba ↑** y la nave se moverá más rápido hacia arriba.
